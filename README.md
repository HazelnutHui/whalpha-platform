...
The offline [EOD historical-window boundary](docs/architecture/eod-historical-window.md) now plans the 20 XNYS sessions before an analysis date, validates bounded canonical partitions, and calculates an exact 20-session median dollar-volume proxy only with 20/20 observations. The 2026-08-14 window has two completed sessions and 18 missing, so readiness is `insufficient_history`; no production metric or Universe change occurred.
