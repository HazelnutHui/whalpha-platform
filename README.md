...
Phase B2A added the offline SEC issuer-structure boundary. Phase B2B added the bounded streaming transport, source-cache safety checks, normalized observation persistence, canonical evidence persistence, and logical completion contract. Four separately authorized bounded runs each stopped after three requests and zero retries at Series/Class landing discovery; none published a completed SEC cache or evidence snapshot. The fourth run's schema `2.0` evidence supports one exact 2024 Series/Class legacy path, now covered offline without inferring any 2023-or-earlier rule. Diagnostic aggregate counts are derived from candidate records. No additional live run has exercised this correction. See [SEC Issuer-Structure Evidence](docs/architecture/sec-issuer-structure-evidence.md) and the [B2B run audit](docs/audits/sec-issuer-structure-evidence-2026-08-14.md).
