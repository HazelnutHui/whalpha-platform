"""Immutable owner-only custody for Factor Catalog V2 screening results."""
...
REPORT_FILE = "factor-screening-v2-report.json"
OUTPUT_PREFIX = "report="
MAXIMUM_REPORT_BYTES = 16 * 1024 * 1024


class QuantResearchFactorScreeningV2PersistenceError(RuntimeError):
    """Raised when private V2 factor-screening custody cannot be trusted."""


def write_quant_research_factor_screening_v2_report(
    *,
    output_root: Path,
    output_custody_root: Path,
    report: QuantResearchFactorScreeningReportV2,
) -> tuple[Path, str, str]:
    target, custody = _validated_target(output_root, output_custody_root)
    if target.exists() or target.is_symlink():
        existing, sha256 = read_quant_research_factor_screening_v2_report(
            output_root=target,
            output_custody_root=custody,
        )
        if existing != report:
            raise QuantResearchFactorScreeningV2PersistenceError(
                "existing V2 factor-screening report differs"
            )
    ...
    try:
        ...
        payload = factor_screening_v2_report_bytes(report)
        _write_exclusive(staging / REPORT_FILE, payload)
        staged, _ = _read_report_root(staging)
        if staged != report:
            raise QuantResearchFactorScreeningV2PersistenceError(
                "V2 factor-screening staging reread differs"
            )
        ...
        if isinstance(exc, QuantResearchFactorScreeningV2PersistenceError):
            raise
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening report write failed"
        ) from exc
    reread, sha256 = _read_report_root(target)
    if reread != report:
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening final reread differs"
        )
    return target / REPORT_FILE, sha256, "published"


def read_quant_research_factor_screening_v2_report(
    *, output_root: Path, output_custody_root: Path
) -> tuple[QuantResearchFactorScreeningReportV2, str]:
    target, _ = _validated_target(output_root, output_custody_root)
    return _read_report_root(target)


def factor_screening_v2_report_bytes(
    report: QuantResearchFactorScreeningReportV2,
) -> bytes:
    return (
        json.dumps(
...
def _validated_target(
    ...
    if (
        ...
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening custody differs"
        )
    return target, custody


def _read_report_root(root: Path) -> tuple[QuantResearchFactorScreeningReportV2, str]:
    if (
        root.is_symlink()
        or not root.is_dir()
        ...
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening report directory differs"
        )
    ...
    if (
        ...
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening report file differs"
        )
    payload = path.read_bytes()
    try:
        report = QuantResearchFactorScreeningReportV2.model_validate_json(payload)
    except Exception as exc:
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening report is invalid"
        ) from exc
    if payload != factor_screening_v2_report_bytes(report):
        raise QuantResearchFactorScreeningV2PersistenceError(
            "V2 factor-screening report bytes are not canonical"
        )
