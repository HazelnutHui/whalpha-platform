"""Exact-authorization CLI for the six-page inactive lifecycle census."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        required = required_lifecycle_census_acknowledgement(
            anchor_date=args.anchor_date, revision=revision
        )
        if args.review:
            print(f"revision={revision}")
            print(f"anchor_date={args.anchor_date.isoformat()}")
            print(f"maximum_request_count={CENSUS_MAXIMUM_REQUEST_COUNT}")
