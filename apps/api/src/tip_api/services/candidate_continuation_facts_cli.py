"""Offline CLI for the descriptive Candidate continuation-fact audit."""
...
from tip_api.services.candidate_continuation_facts_audit import (
    ...
    validate_continuation_facts_tmp_output_dir,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a shadow-only Candidate continuation-fact audit in /tmp."
    )
    ...
    if args.verify_output:
        manifest = read_candidate_continuation_facts_audit(args.output_dir)
        print(json.dumps(_summary(manifest, args.output_dir), sort_keys=True))
        return 0

    validate_continuation_facts_tmp_output_dir(args.output_dir)
    ...
    with _offline_socket_guard():
        ...
        timings["candidate_current_projection_seconds"] = time.perf_counter() - stage
        ...
        if (
            ...
            raise RuntimeError(
                "continuation-fact current source batches are not complete and Primary-first"
            )
        ...
        for candidate_batch, entry_batch in zip(
            ...
            if oracle.mismatch_count or not oracle.input_permutation_match:
                raise RuntimeError(
                    "continuation-fact Oracle or permutation gate failed"
                )
            output_batches.append(result)
            oracles.append(oracle)
        timings["fact_calculation_and_oracle_seconds"] = time.perf_counter() - stage

        stage = time.perf_counter()
        panel_entry = args.panel_cache_root / panel_manifest["cache_key"]
        manifest = write_candidate_continuation_facts_audit(
            ...
            panel_cache_entry_dir=panel_entry,
    ...
    print(
        json.dumps(
            {**_summary(manifest, args.output_dir), "timings": timings},
            sort_keys=True,
        )
    )
    return 0
...
def _read_records(path: Path) -> list[dict]:
    if path.is_symlink() or not path.is_file():
        raise RuntimeError("continuation-fact source record artifact is unsafe")
    payload = json.loads(path.read_bytes())
    records = payload.get("records") if isinstance(payload, dict) else None
    if not isinstance(records, list) or not all(
        isinstance(item, dict) for item in records
    ):
        raise RuntimeError("continuation-fact source record payload is malformed")
    return records
...
def _summary(manifest, output_dir):
    return {
        ...
        "assessed_count": manifest.get("assessed_count"),
...
def _offline_socket_guard():
    ...
    class GuardedSocket(original_socket):
        def connect(self, *args, **kwargs):  # type: ignore[no-untyped-def]
            raise RuntimeError("network is prohibited during continuation-fact audit")
