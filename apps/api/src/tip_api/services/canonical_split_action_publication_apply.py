"""Atomic, inventory-bound Apply for one canonical split-action publication."""
...
from tip_api.contracts.market_data.v1 import CanonicalSplitActionPublicationV1
...
class CanonicalSplitActionPublicationApplyError(RuntimeError):
    """Raised when canonical split actions cannot be published safely."""


@dataclass(frozen=True, slots=True)
class CanonicalSplitActionPublicationApplyResult:
    status: str
    plan_sha256: str
    ...
    active_action_record_count: int
    quarantined_action_record_count: int
...
def apply_approved_canonical_split_action_publication_plan(
    *,
    plan_path: Path,
...
) -> CanonicalSplitActionPublicationApplyResult:
    ...
    try:
        ...
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action publication lock is unavailable"
        ) from exc
    ...
    with os.fdopen(descriptor, "r+b") as lock:
        ...
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_uid != os.getuid():
            raise CanonicalSplitActionPublicationApplyError(
                "canonical split-action publication lock custody differs"
            )
        ...
        if locked != evidence:
            raise CanonicalSplitActionPublicationApplyError(
                "canonical split-action plan changed before locked execution"
            )
        _validate_binding(
            evidence=locked,
            expected_plan_logical_fingerprint=expected_plan_logical_fingerprint,
        ...
        with _network_prohibited():
            if os.path.lexists(target):
                ...
                outside = inventory_reader(
                    root, exclude_prefixes=(target,)
                )
    ...
    return CanonicalSplitActionPublicationApplyResult(
        status=status,
        ...
        action_record_count=publication.action_record_count,
        active_action_record_count=publication.active_action_record_count,
...
def _read_plan(
    plan_path: Path, approved_plan_sha256: str
) -> CanonicalSplitActionPublicationPlanEvidence:
    try:
    ...
    except CanonicalSplitActionPublicationPlanError as exc:
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action plan failed formal reread"
        ) from exc


def _validate_binding(
    *,
    evidence: CanonicalSplitActionPublicationPlanEvidence,
    expected_plan_logical_fingerprint: str,
    expected_current_state_fingerprint: str,
    data_root: Path,
) -> None:
    ...
    if (
        ...
        or plan.operation != "publish_canonical_split_actions"
        ...
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action Apply binding differs"
        )
...
def _publish(
    *,
    ...
    if os.path.lexists(target) or os.path.lexists(staging):
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action target or staging path already exists"
        )
    ...
    try:
        for artifact in evidence.plan.artifacts:
            ...
            if (
                ...
                raise CanonicalSplitActionPublicationApplyError(
                    "staged canonical split-action artifact differs"
                )
        _fsync_directory(staging)
        if os.path.lexists(target):
            raise CanonicalSplitActionPublicationApplyError(
                "canonical split-action target appeared during staging"
            )
        ...
        if isinstance(exc, CanonicalSplitActionPublicationApplyError):
            raise
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action atomic publication failed"
        ) from exc
...
def _verify_target(
    root: Path,
    target: Path,
    expected: CanonicalSplitActionPublicationV1,
) -> CanonicalSplitActionPublicationRead:
    try:
        canonical = read_canonical_split_action_publication(
            data_root=root, publication_root=target
        )
    except CanonicalSplitActionPersistenceError as exc:
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action publication failed formal reread"
        ) from exc
    if canonical.publication != expected:
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action publication differs from plan"
        )
    return canonical
...
def _verify_source(path: Path, size: int, sha256: str) -> None:
    if path.is_symlink() or not path.is_file():
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action source artifact is unavailable"
        )
    metadata = path.stat()
    if (
        ...
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action source artifact differs from plan"
        )
...
def _mkdirs_durable(path: Path, root: Path) -> None:
    ...
    while not current.exists():
        if current == root or root not in current.parents:
            raise CanonicalSplitActionPublicationApplyError(
                "canonical split-action path escaped the data root"
            )
        missing.append(current)
        current = current.parent
    if current.is_symlink() or not current.is_dir():
        raise CanonicalSplitActionPublicationApplyError(
            "canonical split-action publication parent is unsafe"
        )
...
def _reject_symlink_chain(root: Path, target: Path) -> None:
    ...
    while True:
        if current.is_symlink():
            raise CanonicalSplitActionPublicationApplyError(
                "canonical split-action publication path contains a symlink"
            )
        if current == root:
            return
        current = current.parent
...
def _validate_fingerprint(value: str, label: str) -> None:
    if len(value) != _SHA256_LENGTH or any(
        ...
        raise CanonicalSplitActionPublicationApplyError(f"{label} is malformed")
...
def _network_prohibited() -> Iterator[None]:
    ...
    def blocked(*_args: object, **_kwargs: object) -> None:
        raise CanonicalSplitActionPublicationApplyError(
            "network is prohibited during canonical split-action Apply"
        )
