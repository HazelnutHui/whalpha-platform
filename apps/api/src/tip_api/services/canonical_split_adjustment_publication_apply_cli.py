"""Explicit CLI for one exact canonical split-adjustment publication Apply."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Apply one exact canonical split-adjustment publication plan."
    )
    ...
    try:
        result = apply_approved_canonical_split_adjustment_publication_plan(
        ...
        print(
            json.dumps(
                {
                    "status": "rejected",
                    "reason_code": "canonical_split_adjustment_apply_rejected",
