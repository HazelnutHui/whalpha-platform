"""CLI for the read-only canonical split coverage diagnostic."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Scan canonical EOD for split-coverage contradictions without "
            "writing or inferring corporate actions."
        )
    )
    ...
    try:
        report = diagnose_canonical_split_coverage(
        ...
        print(
            json.dumps(
                {
                    "status": "rejected",
                    "reason_code": "canonical_split_coverage_diagnostic_rejected",
    ...
    payload["flags"] = [
        item.as_dict() for item in report.flags[: args.maximum_output_flags]
    ]
    payload["output_flag_count"] = len(payload["flags"])
    payload["output_flags_truncated"] = len(report.flags) > len(payload["flags"])
