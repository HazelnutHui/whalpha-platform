"""Pure Development label mechanics for Factor Catalog V1 screening."""
...
class QuantResearchFactorScreeningLabelError(ValueError):
    """Raised when one factor-screening label cannot be constructed safely."""


def build_quant_research_factor_screening_label(
    *,
    ...
    """Build one factor-bound label without reading prior strategy outcomes."""

    horizon = len(expected_path_sessions)
    if horizon not in {1, 3, 5}:
        raise QuantResearchFactorScreeningLabelError(
            "factor-screening horizon must be 1, 3, or 5 sessions"
        )
    if len(instrument_bars) != horizon or len(benchmark_bars) != horizon:
        raise QuantResearchFactorScreeningLabelError(
            "factor-screening bars must cover the declared horizon"
        )
    ...
    for bar in (
        ...
        *benchmark_bars,
        ...
        _validate_bar(bar)
    common = {
        "protocol_fingerprint": quant_research_factor_screening_protocol_v1().logical_fingerprint,
        "catalog_fingerprint": quant_research_factor_screening_protocol_v1().catalog_fingerprint,
    ...
    if unavailable_reason_codes:
        return build_factor_screening_label(
            **common,
...
def _validate_bar(bar: ReconstructedOutcomeBarV1) -> None:
    ...
    if (
        ...
        raise QuantResearchFactorScreeningLabelError(
            "factor-screening label bar is invalid"
        )
