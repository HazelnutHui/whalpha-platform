"""Resumable private custody for listed-consideration 424B3 documents."""
...
CONTRACT_VERSION = "strong-leader-pullback-listed-consideration-source/1.0"
ARTIFACT_VERSION = "strong-leader-pullback-listed-consideration-artifact/1.0"
...
class StrongLeaderPullbackListedConsiderationSourceError(RuntimeError):
    """Raised when listed-consideration source custody cannot be proven."""
...
class ListedConsiderationDocumentArtifactV1(_FrozenModel):
    contract_version: Literal[
        "strong-leader-pullback-listed-consideration-artifact/1.0"
    ] = ARTIFACT_VERSION
    ...
    registration_accession_number: str = Field(
    ...
    identity_assignment_count: Literal[0] = 0
    ...
    @field_validator("request_url", "content_type")
    @classmethod
    def text_is_normalized(cls, value: str) -> str:
        if not value or value != value.strip():
            raise ValueError("listed-consideration artifact text differs")
        return value

    @model_validator(mode="after")
    def artifact_reconciles(self) -> "ListedConsiderationDocumentArtifactV1":
        if (
            self.content_type not in _ALLOWED_CONTENT_TYPES
            or self.logical_fingerprint
            != _fingerprint(
                self.model_dump(mode="json", exclude={"logical_fingerprint"})
            )
        ):
            raise ValueError("listed-consideration artifact differs")
        return self


class StrongLeaderPullbackListedConsiderationSourceManifestV1(_FrozenModel):
    contract_version: Literal[
        "strong-leader-pullback-listed-consideration-source/1.0"
    ] = CONTRACT_VERSION
    ...
    planned_document_count: Literal[12] = EXPECTED_CASE_COUNT
    completed_document_count: Literal[12] = EXPECTED_CASE_COUNT
    external_request_count: int = Field(ge=EXPECTED_CASE_COUNT)
    retry_count: int = Field(ge=0)
    total_document_bytes: int = Field(ge=EXPECTED_CASE_COUNT)
    ...
    def revisions_are_ordered_unique(cls, value: object) -> tuple[object, ...]:
        ...
        if (
            ...
            raise ValueError("listed-consideration source revisions differ")
        return values

    @model_validator(mode="after")
    def manifest_reconciles(self) -> "StrongLeaderPullbackListedConsiderationSourceManifestV1":
        if (
            self.first_observed_at > self.completed_at
            ...
            raise ValueError("listed-consideration source manifest differs")
        return self


@dataclass(frozen=True, slots=True)
class StrongLeaderPullbackListedConsiderationSourceResult:
    output_root: Path
    status: Literal["in_progress", "published", "already_present"]
    completed_document_count: int
    new_document_count: int
    network_request_count: int
    manifest: StrongLeaderPullbackListedConsiderationSourceManifestV1 | None = None
...
ProgressCallback = Callable[[ListedConsiderationDocumentArtifactV1, int, int], None]
Clock = Callable[[], datetime]


def acquire_strong_leader_pullback_listed_consideration_source(
    *,
    plan_root: Path,
    ...
    if re.fullmatch(_REVISION_PATTERN, implementation_revision) is None:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source revision is invalid"
        )
    if maximum_new_documents is not None and not (
        1 <= maximum_new_documents <= EXPECTED_CASE_COUNT
    ):
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source run bound is invalid"
        )
    if (
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source configuration exceeds policy"
        )
    ...
    if target.exists() or target.is_symlink():
        if partial.exists() or partial.is_symlink():
            raise StrongLeaderPullbackListedConsiderationSourceError(
                "completed and partial listed-consideration sources coexist"
            )
        existing = read_strong_leader_pullback_listed_consideration_source(
            plan_root=plan_root,
            plan_custody_root=plan_custody_root,
            output_root=target,
            output_custody_root=output_custody_root,
        )
        return StrongLeaderPullbackListedConsiderationSourceResult(
            output_root=target,
            status="already_present",
            completed_document_count=EXPECTED_CASE_COUNT,
    ...
    decisions = {item.request_sequence: item for item in plan.report.decisions}
    artifacts = _read_completed_artifacts(partial, decisions)
    ...
    new_count = 0
    network_count = 0
    for sequence in sorted(decisions):
        if sequence in artifacts:
            continue
        ...
        if progress is not None:
            progress(artifact, len(artifacts), EXPECTED_CASE_COUNT)
    ordered = tuple(artifacts[index] for index in sorted(decisions))
    manifest = _build_manifest(plan=plan, artifacts=ordered, completed_at=now())
    _write_exclusive(
        partial / MANIFEST_FILE, _json_bytes(manifest.model_dump(mode="json"))
    )
    _fsync_directory(partial)
    partial.replace(target)
    _fsync_directory(target.parent)
    reread = read_strong_leader_pullback_listed_consideration_source(
        plan_root=plan_root,
        plan_custody_root=plan_custody_root,
        output_root=target,
        output_custody_root=output_custody_root,
...
def read_strong_leader_pullback_listed_consideration_source(
    *,
    plan_root: Path,
    plan_custody_root: Path,
    output_root: Path,
    ...
    expected_names = {
        MANIFEST_FILE,
        *(f"request={item.request_sequence:06d}" for item in plan.report.decisions),
    }
    if {item.name for item in root.iterdir()} != expected_names:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source members differ"
        )
    manifest_path = root / MANIFEST_FILE
    _require_regular_file(manifest_path, 0o400, MAXIMUM_MANIFEST_BYTES)
    raw = manifest_path.read_bytes()
    try:
        manifest = StrongLeaderPullbackListedConsiderationSourceManifestV1.model_validate_json(
            raw
        )
    except Exception as exc:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source manifest is invalid"
        ) from exc
    if (
        manifest.plan_report_sha256 != plan.report_sha256
        or manifest.plan_logical_fingerprint != plan.report.logical_fingerprint
        or raw != _json_bytes(manifest.model_dump(mode="json"))
    ):
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source plan binding differs"
        )
    artifacts = tuple(
        _read_artifact_directory(
            root / f"request={item.request_sequence:06d}", item
        )
        for item in plan.report.decisions
    )
    _validate_manifest_aggregates(manifest, artifacts)
    return StrongLeaderPullbackListedConsiderationSourceResult(
        output_root=root,
        status="already_present",
        completed_document_count=EXPECTED_CASE_COUNT,
...
def _prepare_partial(
    *,
    ...
    if partial.is_symlink():
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration partial custody is unsafe"
        )
    ...
    valid_sequences = {item.request_sequence for item in plan.report.decisions}
    for member in tuple(partial.iterdir()):
        ...
        if request_match is not None:
            if int(request_match.group("sequence")) not in valid_sequences:
                raise StrongLeaderPullbackListedConsiderationSourceError(
                    "listed-consideration partial request is unplanned"
                )
            continue
        ...
        if staging_match is not None:
            ...
            if (
                ...
                raise StrongLeaderPullbackListedConsiderationSourceError(
                    "listed-consideration staging member is unsafe"
                )
            shutil.rmtree(member)
            _fsync_directory(partial)
            continue
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration partial members differ"
        )
...
def _adopt_completed_partial(
    *,
    ...
    if {item.name for item in partial.iterdir()} != expected_names:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "completed listed-consideration partial members differ"
        )
    ...
    try:
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "completed listed-consideration partial manifest is invalid"
        ) from exc
    if (
        manifest.plan_report_sha256 != plan.report_sha256
        or manifest.plan_logical_fingerprint != plan.report.logical_fingerprint
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "completed listed-consideration partial plan binding differs"
        )
    artifacts = tuple(
        _read_artifact_directory(
            partial / f"request={item.request_sequence:06d}", item
        )
        for item in plan.report.decisions
    )
    _validate_manifest_aggregates(manifest, artifacts)
...
def _read_completed_artifacts(
    partial: Path, decisions: dict[int, ListedConsiderationSourcePlanDecisionV1]
) -> dict[int, ListedConsiderationDocumentArtifactV1]:
    artifacts: dict[int, ListedConsiderationDocumentArtifactV1] = {}
    for member in sorted(partial.iterdir()):
        match = _REQUEST_NAME_PATTERN.fullmatch(member.name)
        if match is None:
            raise StrongLeaderPullbackListedConsiderationSourceError(
                "listed-consideration partial request name differs"
            )
        ...
        artifacts[sequence] = _read_artifact_directory(member, decision)
    return artifacts
...
def _download_decision(
    *,
    ...
    final = partial / f"request={decision.request_sequence:06d}"
    staging = partial / f".request={decision.request_sequence:06d}.partial"
    if final.exists() or final.is_symlink() or staging.exists() or staging.is_symlink():
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration download target exists"
        )
    staging.mkdir(mode=0o700)
    try:
        result = transport.download(
            decision.registration_document_url,
            staging / DOCUMENT_FILE,
        ...
        if (
            result.url != decision.registration_document_url
            or result.byte_count < 1
            or result.byte_count > MAXIMUM_DOCUMENT_BYTES
            ...
            raise StrongLeaderPullbackListedConsiderationSourceError(
                "listed-consideration response differs from the plan"
            )
        (staging / DOCUMENT_FILE).chmod(0o400)
        values = {
            "implementation_revision": implementation_revision,
            "request_sequence": decision.request_sequence,
            "plan_decision_fingerprint": decision.logical_fingerprint,
            ...
            "registration_accession_number": (
                decision.registration_accession_number
            ),
            "request_url": decision.registration_document_url,
        ...
        artifact = ListedConsiderationDocumentArtifactV1.model_validate(
            {
                **values,
...
def _read_artifact_directory(
    directory: Path, decision: ListedConsiderationSourcePlanDecisionV1
) -> ListedConsiderationDocumentArtifactV1:
    if (
        ...
        or directory.name != f"request={decision.request_sequence:06d}"
    ):
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration artifact directory differs"
        )
    if {item.name for item in directory.iterdir()} != {ARTIFACT_FILE, DOCUMENT_FILE}:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration artifact members differ"
        )
    ...
    try:
        artifact = ListedConsiderationDocumentArtifactV1.model_validate_json(
            artifact_path.read_bytes()
        )
    except Exception as exc:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration artifact is invalid"
        ) from exc
    if (
        artifact.request_sequence != decision.request_sequence
        or artifact.plan_decision_fingerprint != decision.logical_fingerprint
        or artifact.target_instrument_id != decision.target_instrument_id
        or artifact.proposed_consideration_instrument_id
        != decision.proposed_consideration_instrument_id
        or artifact.proposed_cik != decision.proposed_cik
        or artifact.registration_accession_number
        != decision.registration_accession_number
        or artifact.request_url != decision.registration_document_url
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration artifact binding differs"
        )
    return artifact
...
def _build_manifest(
    *,
    plan: StrongLeaderPullbackListedConsiderationSourcePlanResult,
    artifacts: tuple[ListedConsiderationDocumentArtifactV1, ...],
    completed_at: datetime,
) -> StrongLeaderPullbackListedConsiderationSourceManifestV1:
    if len(artifacts) != EXPECTED_CASE_COUNT:
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source completion count differs"
        )
    values = {
        ...
        "content_type_counts": tuple(
            sorted(Counter(item.content_type for item in artifacts).items())
        ),
        "artifact_binding_fingerprint": _fingerprint(
            tuple(item.model_dump(mode="json") for item in artifacts)
        ),
    }
    provisional = StrongLeaderPullbackListedConsiderationSourceManifestV1.model_construct(
        **values, logical_fingerprint="0" * 64
    )
    return StrongLeaderPullbackListedConsiderationSourceManifestV1.model_validate(
        {
            **values,
            "logical_fingerprint": _fingerprint(
                provisional.model_dump(mode="json", exclude={"logical_fingerprint"})
...
def _validate_manifest_aggregates(
    manifest: StrongLeaderPullbackListedConsiderationSourceManifestV1,
    artifacts: tuple[ListedConsiderationDocumentArtifactV1, ...],
) -> None:
    if (
        tuple(item.request_sequence for item in artifacts)
        != tuple(sorted(item.request_sequence for item in artifacts))
        ...
        or manifest.completed_at < max(item.observed_at for item in artifacts)
        ...
        != tuple(sorted(Counter(item.content_type for item in artifacts).items()))
        or manifest.artifact_binding_fingerprint
        != _fingerprint(tuple(item.model_dump(mode="json") for item in artifacts))
    ):
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source aggregates differ"
        )
...
def _validated_output_target(path: Path, custody_root: Path) -> Path:
    if not path.is_absolute() or not custody_root.is_absolute():
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source paths must be absolute"
        )
    ...
    if (
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source custody or target is unsafe"
        )
    return path
...
def _validated_completed_output(path: Path, custody_root: Path) -> Path:
    ...
    if (
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source output is unavailable or unsafe"
        )
    return target
...
def _require_regular_file(path: Path, mode: int, maximum_bytes: int) -> None:
    if path.is_symlink() or not path.is_file():
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source file is unavailable or unsafe"
        )
    metadata = path.stat()
    if (
        ...
        raise StrongLeaderPullbackListedConsiderationSourceError(
            "listed-consideration source file metadata differs"
        )
