"""CLI for resumable first-strategy SEC primary-document custody."""
...
from tip_api.services.strong_leader_pullback_sec_document_source import (
    ...
    StrongLeaderPullbackSecDocumentSourceError,
    acquire_strong_leader_pullback_sec_document_source,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    def progress(
        artifact: SecPrimaryDocumentArtifactV1,
        completed: int,
        total: int,
    ) -> None:
        print(
            json.dumps(
                {
                    ...
                    "record_type": "sec_document_source_checkpoint",
    ...
    try:
        ...
        result = acquire_strong_leader_pullback_sec_document_source(
            plan_root=args.plan,
        ...
        print(
            json.dumps(
                {
                    "contract_version": CONTRACT_VERSION,
                    "record_type": "sec_document_source_stop",
                    "status": "stopped",
                    "implementation_revision": revision,
    ...
    output = {
        "contract_version": CONTRACT_VERSION,
        "record_type": "sec_document_source_progress",
    ...
    if result.manifest is not None:
        output.update(
            {
                "record_type": "sec_document_source_completion",
