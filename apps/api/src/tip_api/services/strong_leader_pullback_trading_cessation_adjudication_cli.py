"""CLI for first-strategy SEC trading-cessation evidence."""

from __future__ import annotations

import argparse
import json
import subprocess
import sys
from datetime import datetime
from pathlib import Path

from tip_api.contracts.common import normalize_utc_datetime
from tip_api.services.strong_leader_pullback_trading_cessation_adjudication import (
    StrongLeaderPullbackTradingCessationAdjudicationError,
    build_strong_leader_pullback_trading_cessation_adjudication,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-sample", required=True, type=Path)
    parser.add_argument("--source-sample-custody-root", required=True, type=Path)
    parser.add_argument("--form25", required=True, type=Path)
    parser.add_argument("--form25-custody-root", required=True, type=Path)
    parser.add_argument("--termination-reasons", required=True, type=Path)
    parser.add_argument("--termination-reasons-custody-root", required=True, type=Path)
    parser.add_argument("--party-relations", required=True, type=Path)
    parser.add_argument("--party-relations-custody-root", required=True, type=Path)
    parser.add_argument("--eod-root", required=True, type=Path)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--output-custody-root", required=True, type=Path)
    parser.add_argument("--evaluated-at", required=True, type=_datetime)
    parser.add_argument("--execute", action="store_true")
    args = parser.parse_args(argv)
    if not args.execute:
        parser.error("--execute is required")
    try:
        result = build_strong_leader_pullback_trading_cessation_adjudication(
            source_sample_root=args.source_sample,
            source_sample_custody_root=args.source_sample_custody_root,
            form25_root=args.form25,
            form25_custody_root=args.form25_custody_root,
            termination_reason_root=args.termination_reasons,
            termination_reason_custody_root=args.termination_reasons_custody_root,
            party_relation_root=args.party_relations,
            party_relation_custody_root=args.party_relations_custody_root,
            eod_root=args.eod_root,
            output_root=args.output_root,
            output_custody_root=args.output_custody_root,
            implementation_revision=_clean_revision(),
            evaluated_at=args.evaluated_at,
        )
    except (OSError, RuntimeError, ValueError) as exc:
        print(
            json.dumps(
                {
                    "status": "stopped",
                    "error_type": type(exc).__name__,
                    "network_request_count": 0,
                    "canonical_lifecycle_fact_count": 0,
                    "terminal_outcome_count": 0,
                    "canonical_data_write_count": 0,
                    "research_admission_count": 0,
                },
                sort_keys=True,
                separators=(",", ":"),
            ),
            file=sys.stderr,
        )
        return 1
    report = result.report
    print(
        json.dumps(
            {
                "contract_version": report.contract_version,
                "status": result.status,
                "completion_status": report.completion_status,
                "implementation_revision": report.implementation_revision,
                "evaluated_at": report.evaluated_at.isoformat(),
                "lifecycle_case_count": report.lifecycle_case_count,
                "matched_cessation_count": report.matched_cessation_count,
                "conflicting_cessation_count": report.conflicting_cessation_count,
                "unsupported_cessation_count": report.unsupported_cessation_count,
                "selected_eod_session_count": report.selected_eod_session_count,
                "report_sha256": result.report_sha256,
                "logical_fingerprint": report.logical_fingerprint,
                "network_request_count": 0,
                "canonical_lifecycle_fact_count": 0,
                "terminal_outcome_count": 0,
                "canonical_data_write_count": 0,
                "research_admission_count": 0,
            },
            sort_keys=True,
            separators=(",", ":"),
        )
    )
    return 0


def _datetime(value: str) -> datetime:
    try:
        return normalize_utc_datetime(datetime.fromisoformat(value))
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "timestamp must be an ISO-8601 UTC value"
        ) from exc


def _clean_revision() -> str:
    status = subprocess.run(
        ["git", "status", "--porcelain"],
        check=True,
        capture_output=True,
        text=True,
    ).stdout
    if status:
        raise StrongLeaderPullbackTradingCessationAdjudicationError(
            "repository must be clean"
        )
    return subprocess.run(
        ["git", "rev-parse", "HEAD"],
        check=True,
        capture_output=True,
        text=True,
    ).stdout.strip()


if __name__ == "__main__":  # pragma: no cover
    raise SystemExit(main())
