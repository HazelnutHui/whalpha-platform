"""Explicit CLI for one approved Membership Apply or recovery."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply one exact Membership plan, or verify and complete an "
            "interrupted physical-first/marker-last invocation."
        )
    )
    ...
    try:
        result = apply_approved_universe_membership_plan(
        ...
        print(
            json.dumps(
                {
                    "status": "rejected",
                    "reason_code": "universe_membership_apply_rejected",
