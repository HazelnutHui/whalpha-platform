...
def test_scripts_default_dry_run_and_nginx_template(repo_root: Path = Path(__file__).resolve().parents[4]):
    ...
    assert 'value["snapshot_manifest_sha256"]' in inspect_script.read_text()
    assert '"${guest_private_sha}" == "${snapshot_manifest_sha}"' in inspect_script.read_text()
    assert "guest_release" not in inspect_script.read_text()
