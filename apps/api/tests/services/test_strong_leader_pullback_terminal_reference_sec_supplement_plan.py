...
from tip_api.services import (
    strong_leader_pullback_terminal_reference_sec_supplement_plan as service,
)
...
def test_builds_exact_two_document_supplement(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    package = _package(tmp_path, monkeypatch)
    report = (
        service.build_strong_leader_pullback_terminal_reference_sec_supplement_plan(
            submissions_package_path=package,
            submissions_custody_root=package.parent,
            implementation_revision="a" * 40,
            planned_at=datetime(2026, 9, 15, 2, 30, tzinfo=UTC),
        )
    )

    assert report.completion_status == "planned_not_executed"
    assert tuple(item.ticker_locator for item in report.items) == ("REVG", "SKX")
    assert report.planned_request_count == 2
    assert report.external_request_count == 0
    assert report.outcome_count == 0


def test_rejects_supplement_accession_metadata_drift(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    package = _package(
        tmp_path,
    ...
    with pytest.raises(service.TerminalReferenceSecSupplementPlanError):
        service.build_strong_leader_pullback_terminal_reference_sec_supplement_plan(
            submissions_package_path=package,
            submissions_custody_root=package.parent,
            implementation_revision="a" * 40,
...
def test_publishes_and_rereads_owner_only_supplement(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    package = _package(tmp_path, monkeypatch)
    ...
    result = (
        service.publish_strong_leader_pullback_terminal_reference_sec_supplement_plan(
            **arguments
        )
    )
    repeated = (
...
def test_supplement_item_rejects_request_url_drift() -> None:
    registered = service._REQUESTS[0]
    with pytest.raises(ValidationError, match="request URL differs"):
        service.base.TerminalReferenceSecPlanItemV1.model_validate(
            {
                **registered,
                "request_sequence": 1,
