...
describe('Market Regime Opportunity Map desktop preview', () => {
  ...
  it('renders the hero, reconciled dimensions, all pairs and short-history boundary', async () => {
    render(<MarketRegimeOpportunityMapPage />);
    expect(await screen.findByText('63.9102')).toBeInTheDocument();
  ...
  it('switches windows, filters, preserves relative spread and opens detail', async () => {
    render(<MarketRegimeOpportunityMapPage />); await screen.findByText('63.9102');
  ...
  it('isolates Universe state while keeping ETF metrics and supports history navigation', async () => {
    ...
    expect(await screen.findByText('64.8167')).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByText('63.9102')).toBeInTheDocument());
  });
