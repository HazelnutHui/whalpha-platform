...
type CandidateView = 'entry' | 'leadership';
...
export function OpportunityCandidatesPage(): JSX.Element {
  ...
  const [state, setState] = useState<LoadState>({ kind: 'loading' }); const [stage, setStage] = useState<StageFilter>('active'); const [search, setSearch] = useState(''); const [selected, setSelected] = useState<CandidateItem | null>(null); const [view, setView] = useState<CandidateView>('entry'); const [lane, setLane] = useState<CandidateEntryLane>('review_now');
  ...
  return <main className="app-shell candidate-shell"><section className="candidate-hero"><p className="eyebrow">{t('candidate.eyebrow')}</p><h1>{t('candidate.title')}</h1><p>{t('candidate.subtitleV2')}</p><div className="candidate-hero-facts"><span>{t('common.asOf')} <strong>{data.as_of_session}</strong></span><span>{universeName(t, data.selected_universe_id)} <strong>{data.universe.universe_member_count.toLocaleString('en-US')}</strong></span><span>{t('candidate.hardQualified')} <strong>{entryMode?.hard_risk_gate_qualified_count ?? risk.eligible_count}</strong></span><span>{t('candidate.displayed')} <strong>{rows.length}</strong></span></div></section>
    <section className="panel candidate-control-panel"><div className="candidate-risk-tabs" aria-label={t('candidate.riskMode')}>{MODES.map((value) => <button type="button" className={mode === value ? 'active' : ''} key={value} onClick={() => { setMode(value); writeQuery('candidateRisk', value); }}>{t(`candidate.risk.${value}` as never)}</button>)}</div><p>{t('candidate.riskExplanationV2')}</p><div className="candidate-view-tabs"><button type="button" className={view === 'entry' ? 'active' : ''} disabled={!entryMode} onClick={() => setView('entry')}>{t('candidate.entry.view')}</button><button type="button" className={view === 'leadership' ? 'active' : ''} onClick={() => setView('leadership')}>{t('candidate.leadership.view')}</button></div><div className="candidate-filters">{view === 'leadership' ? <label>{t('common.state')}<select value={stage} onChange={(event) => setStage(event.target.value as StageFilter)}><option value="active">{t('candidate.filter.active')}</option><option value="watch">{t('candidate.stage.watch')}</option><option value="prepare">{t('candidate.stage.prepare')}</option><option value="enter">{t('candidate.stage.enter')}</option><option value="invalidated">{t('candidate.stage.invalidated')}</option><option value="all">{t('candidate.filter.all')}</option></select></label> : null}<label>{t('candidate.search')}<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="AAPL" /></label></div></section>
    ...
    <section className={`panel candidate-board ${view === 'entry' ? 'candidate-board-entry' : ''}`}><div className="candidate-table-head"><span>{view === 'entry' ? t('candidate.entry.order') : t('candidate.rank')}</span><span>{t('candidate.symbol')}</span><span>{view === 'entry' ? t('candidate.entry.location') : t('common.state')}</span><span>{view === 'entry' ? t('candidate.entry.structure') : t('candidate.baseScore')}</span><span>{view === 'entry' ? t('candidate.entry.extensionLabel') : t('candidate.dataSupport')}</span><span>{t('candidate.baseScore')}</span><span>{view === 'entry' ? t('candidate.entry.supportDistance') : t('candidate.liquidity')}</span><span>{view === 'entry' ? t('candidate.formalRank') : t('candidate.quality')}</span></div>{rows.map((item, index) => { const disposition = item.risk_dispositions.find((row) => row.risk_mode === mode)!; const entry = item.entry_geometry; return <button type="button" className="candidate-row" key={item.instrument_id} onClick={() => setSelected(item)}><strong>#{view === 'entry' ? index + 1 : disposition.risk_adjusted_rank ?? '—'}</strong><span><strong>{item.ticker}</strong><small>{item.security_type}</small></span><span className={view === 'entry' && entry ? `candidate-entry-posture candidate-entry-posture-${entry.review_posture}` : `candidate-stage candidate-stage-${item.state.final_stage ?? 'unavailable'}`}>{view === 'entry' && entry ? postureName(t, entry.review_posture) : stageName(t, item.state.final_stage)}</span><strong>{view === 'entry' && entry ? setupName(t, entry.technical_setup) : number(item.base_score)}</strong><span>{view === 'entry' && entry ? extensionName(t, entry.extension_risk) : percent(item.confidence.confidence)}</span><strong>{number(item.base_score)}</strong><span>{view === 'entry' && entry ? percent(entry.metrics.reference_support_distance_pct) : money(item.median_dollar_volume_20)}</span><span>{disposition.risk_adjusted_rank ? `#${disposition.risk_adjusted_rank}` : t('candidate.entry.fullPool')}</span></button>; })}{rows.length === 0 ? <div className="candidate-empty"><h3>{t('candidate.empty')}</h3><p>{view === 'entry' && selectedLane?.qualifying_count === 0 ? t('candidate.entry.noQualified') : t('candidate.emptyBody')}</p></div> : null}</section>
