...
describe('Quant Research Lab', () => {
  ...
  it('shows the current V2 qualification pass and historical Development result without implying an active model', async () => {
    ...
    expect(screen.getByText('QUALIFIED · SCREEN DESIGN OPEN')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'From measurement to a decision—without collapsing the evidence.' })).toBeInTheDocument();
    expect(screen.getByText('Factor Discovery').parentElement).toHaveTextContent('CURRENT · V2 SCREEN DESIGN');
    ...
    expect(screen.getByText('Development screen design').parentElement).toHaveTextContent('CURRENT');
    ...
    expect(screen.getByText('Next bounded action').parentElement).toHaveTextContent('REGISTER FINITE DEVELOPMENT SCREEN');
    ...
    expect(screen.getAllByText('Factor Catalog V2')[1].parentElement).toHaveTextContent('Qualified · screen design next');
  ...
  it('renders the complete research boundary in Chinese', () => {
    ...
    expect(screen.getByText('资格通过 · 开放筛选设计')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '从测量到决策，每一层证据都保持独立。' })).toBeInTheDocument();
    expect(screen.getByText('因子发现层').parentElement).toHaveTextContent('当前阶段 · V2 筛选设计');
    ...
    expect(screen.getByText('开发期筛选设计').parentElement).toHaveTextContent('当前工作');
  ...
  it('renders natural quantitative-research terminology in Spanish', () => {
    ...
    expect(screen.getByText('CALIFICADO · DISEÑO DEL FILTRO ABIERTO')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'De la medición a la decisión, sin mezclar las capas de evidencia.' })).toBeInTheDocument();
    expect(screen.getAllByText('Descubrimiento de factores')[0].parentElement).toHaveTextContent('FASE ACTUAL · DISEÑO DEL FILTRO V2');
