...
describe('Quant Research Lab', () => {
  ...
  it('shows Campaign Three as closed without implying an active model or strategy', async () => {
    ...
    expect(screen.getByText('FACTOR DISCOVERY · THREE CAMPAIGNS CLOSED')).toBeInTheDocument();
    expect(screen.getByText('NO CANDIDATE ALPHA · INTAKE REOPENED')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'From measurement to a decision—without collapsing the evidence.' })).toBeInTheDocument();
    expect(screen.getAllByText('Factor Discovery')[0].parentElement).toHaveTextContent('CURRENT · OUTCOME-BLIND INTAKE');
    ...
    expect(screen.getByText('Current stage').parentElement).toHaveTextContent('OUTCOME-BLIND INTAKE');
    ...
    expect(screen.getByText('Current position').parentElement).toHaveTextContent('INTAKE · NO ACTIVE CAMPAIGN');
    ...
    expect(screen.getByText(/US · U.S. equities · FACTOR DISCOVERY · ACTIVE/)).toBeInTheDocument();
    expect(screen.getByText(/CN · China A-shares · DATA FOUNDATION · ACTIVE/)).toBeInTheDocument();
    expect(screen.getAllByText('Retirement')).toHaveLength(2);
    expect(screen.getByText(/Reconcile the complete source census/)).toBeInTheDocument();
  ...
  it('renders the complete research boundary in Chinese', () => {
    ...
    expect(screen.getByText('因子发现 · 三批研究均已关闭')).toBeInTheDocument();
    expect(screen.getByText('无候选 Alpha · 回到结果盲入口')).toBeInTheDocument();
    ...
    expect(screen.getByText('当前阶段').parentElement).toHaveTextContent('结果盲假设入口');
    expect(screen.getByText('累计登记试验').parentElement).toHaveTextContent('17');
    expect(screen.getByText('当前位置').parentElement).toHaveTextContent('假设入口 · 无活动批次');
    ...
    expect(screen.getByText(/CN · 中国 A 股 · 数据基础 · 进行中/)).toBeInTheDocument();
  ...
  it('renders natural quantitative-research terminology in Spanish', () => {
    ...
    expect(screen.getByText('DESCUBRIMIENTO DE FACTORES · TRES CAMPAÑAS CERRADAS')).toBeInTheDocument();
    expect(screen.getByText('SIN ALPHA CANDIDATO · NUEVA ENTRADA')).toBeInTheDocument();
    ...
    expect(screen.getByText('Posición actual').parentElement).toHaveTextContent('ENTRADA · SIN CAMPAÑA ACTIVA');
