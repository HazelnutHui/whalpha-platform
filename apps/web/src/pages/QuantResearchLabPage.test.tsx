...
describe('Quant Research Lab', () => {
  ...
  it('shows Campaign Three as closed without implying an active model or strategy', async () => {
    ...
    expect(screen.getByText('FACTOR DISCOVERY · SOURCE QUALIFICATION')).toBeInTheDocument();
    expect(screen.getByText('4 CARDS FROZEN · 0 NEW TRIALS')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'From measurement to a decision—without collapsing the evidence.' })).toBeInTheDocument();
    expect(screen.getAllByText('Factor Discovery')[0].parentElement).toHaveTextContent('CURRENT · SOURCE QUALIFICATION');
    ...
    expect(screen.getByText('Current stage').parentElement).toHaveTextContent('SOURCE QUALIFICATION');
    ...
    expect(screen.getByText('Current position').parentElement).toHaveTextContent('4 CARDS · 0 NEW TRIALS');
    ...
    expect(screen.getByText(/US · U.S. equities · SOURCE QUALIFICATION · ACTIVE/)).toBeInTheDocument();
  ...
  it('renders the complete research boundary in Chinese', () => {
    ...
    expect(screen.getByText('因子发现 · 来源资格审查')).toBeInTheDocument();
    expect(screen.getByText('4项研究卡已冻结 · 新试验为0')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '从测量到决策，每一层证据都保持独立。' })).toBeInTheDocument();
    expect(screen.getByText('因子发现层').parentElement).toHaveTextContent('当前阶段 · 来源资格审查');
    ...
    expect(screen.getByText('当前阶段').parentElement).toHaveTextContent('来源资格审查');
    expect(screen.getByText('累计登记试验').parentElement).toHaveTextContent('17');
    expect(screen.getByText('当前位置').parentElement).toHaveTextContent('4项研究卡 · 新试验为0');
  ...
  it('renders natural quantitative-research terminology in Spanish', () => {
    ...
    expect(screen.getByText('DESCUBRIMIENTO · CALIFICACIÓN DE FUENTES')).toBeInTheDocument();
    expect(screen.getByText('4 FICHAS CONGELADAS · 0 PRUEBAS NUEVAS')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'De la medición a la decisión, sin mezclar las capas de evidencia.' })).toBeInTheDocument();
    expect(screen.getAllByText('Descubrimiento de factores')[0].parentElement).toHaveTextContent('FASE ACTUAL · CALIFICACIÓN DE FUENTES');
    ...
    expect(screen.getByText('Fase actual').parentElement).toHaveTextContent('CALIFICACIÓN DE FUENTES');
    expect(screen.getAllByText('Pruebas registradas')[0].parentElement).toHaveTextContent('17');
    expect(screen.getByText('Posición actual').parentElement).toHaveTextContent('4 FICHAS · 0 PRUEBAS NUEVAS');
