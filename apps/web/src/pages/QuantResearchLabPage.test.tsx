...
describe('Quant Research Lab', () => {
  ...
  it('shows Campaign Three as registered and unread without implying an active model or strategy', async () => {
    ...
    expect(screen.getByText('FACTOR DISCOVERY · CAMPAIGN THREE')).toBeInTheDocument();
    expect(screen.getByText('REGISTERED · OUTCOMES UNREAD')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'From measurement to a decision—without collapsing the evidence.' })).toBeInTheDocument();
    expect(screen.getByText('Factor Discovery').parentElement).toHaveTextContent('CURRENT · CAMPAIGN THREE REGISTERED');
    ...
    expect(screen.getByText('Current stage').parentElement).toHaveTextContent('DEVELOPMENT ACCESS BOUNDARY');
    ...
    expect(screen.getByText('Current position').parentElement).toHaveTextContent('3 TRIALS · REGISTERED, UNREAD');
    expect(screen.getByText('Stopped before outcomes').parentElement).toHaveTextContent('1 ALPHA DESIGN REJECTED');
    expect(screen.getByText('Next boundary').parentElement).toHaveTextContent('TYPED DEVELOPMENT GRANT');
    ...
    expect(screen.getByText(/05 · Development evaluation/).parentElement).toHaveTextContent('CURRENT GATE');
    ...
    expect(screen.getByText('Cumulative research ledger').parentElement).toHaveTextContent('17 TRIALS · 3 UNREAD');
    ...
    expect(screen.getAllByText('Third factor campaign')[0].parentElement).toHaveTextContent('REGISTERED · UNREAD');
    ...
    expect(screen.getByText('Successor campaign').parentElement).toHaveTextContent('3 TRIALS · REGISTERED, UNREAD');
    ...
    expect(screen.getAllByText('Third factor campaign')[1].parentElement).toHaveTextContent('Registered · outcomes unread');
  ...
  it('renders the complete research boundary in Chinese', () => {
    ...
    expect(screen.getByText('因子发现 · 第三批研究')).toBeInTheDocument();
    expect(screen.getByText('已登记 · 结果尚未读取')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '从测量到决策，每一层证据都保持独立。' })).toBeInTheDocument();
    expect(screen.getByText('因子发现层').parentElement).toHaveTextContent('当前阶段 · 第三批已登记');
    ...
    expect(screen.getByText('当前阶段').parentElement).toHaveTextContent('开发期访问边界');
    expect(screen.getByText('累计登记试验').parentElement).toHaveTextContent('17');
    expect(screen.getByText('当前位置').parentElement).toHaveTextContent('3项试验 · 已登记、未读结果');
    ...
    expect(screen.getByText('累计研究账本').parentElement).toHaveTextContent('共17项 · 其中3项未读结果');
    ...
    expect(screen.getAllByText('第三批因子研究')[0].parentElement).toHaveTextContent('已登记 · 未读结果');
  ...
  it('renders natural quantitative-research terminology in Spanish', () => {
    ...
    expect(screen.getByText('DESCUBRIMIENTO DE FACTORES · TERCERA CAMPAÑA')).toBeInTheDocument();
    expect(screen.getByText('REGISTRADA · RESULTADOS SIN CONSULTAR')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'De la medición a la decisión, sin mezclar las capas de evidencia.' })).toBeInTheDocument();
    expect(screen.getAllByText('Descubrimiento de factores')[0].parentElement).toHaveTextContent('FASE ACTUAL · TERCERA CAMPAÑA REGISTRADA');
    ...
    expect(screen.getByText('Fase actual').parentElement).toHaveTextContent('PUERTA DE ACCESO A DESARROLLO');
    expect(screen.getAllByText('Pruebas registradas')[0].parentElement).toHaveTextContent('17');
    expect(screen.getByText('Posición actual').parentElement).toHaveTextContent('3 PRUEBAS · REGISTRADAS, SIN RESULTADO');
    ...
    expect(screen.getByText('Registro acumulativo de investigación').parentElement).toHaveTextContent('17 PRUEBAS · 3 SIN RESULTADO');
