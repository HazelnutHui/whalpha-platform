...
const ARCHITECTURE_COPY = {
  en: {
    ...
    layers: [
      ['01', 'Factor Discovery', 'CURRENT · V2 DATA REPAIR', 'Catalog V2 has eight outcome-blind measurements and an exactly replayed qualification report. Its source coverage failed the frozen gate, so no factor may read outcomes until the retained five-year split evidence is admitted and the unchanged protocol passes.'],
  ...
  zh: {
    ...
    layers: [
      ['01', '因子发现层', '当前阶段 · V2 数据修复', '因子目录 V2 已登记8项不读取结果的测量，并完成精确重放的资格诊断。由于数据覆盖未通过冻结门槛，在五年拆股证据获准并以原协议重跑通过前，任何因子都不得读取结果。'],
  ...
  es: {
    ...
    layers: [
      ['01', 'Descubrimiento de factores', 'FASE ACTUAL · REPARACIÓN DE DATOS V2', 'El Catálogo V2 registra ocho mediciones sin resultados y cuenta con una calificación reproducida exactamente. La cobertura no superó el filtro congelado, por lo que ningún factor puede consultar resultados hasta admitir la evidencia histórica de splits y aprobar el mismo protocolo sin cambios.'],
...
const COPY = {
  en: {
    ...
    status: 'FACTOR DISCOVERY V2', statusValue: 'DATA QUALIFICATION BLOCKED',
    statusNote: 'Eight outcome-blind definitions and one exact replay are complete. The frozen 90% coverage gate failed because canonical split evidence starts after the registered 127-session source window. No V2 outcome has been read and no factor is admitted.',
    ...
    readiness: 'Evidence ladder', readinessItems: [
      ...
      ['Cumulative research ledger', 'CURRENT', 'V1 failures and its one retained risk guard remain counted; V2 is explicitly adaptive to that consumed Development evidence.', 'met'],
      ...
      ['V2 data qualification', 'REJECTED', 'The report and replay match exactly, but only 60.89% of vectors were complete under the frozen split-evidence rule.', 'locked'],
      ['Historical split repair', 'CURRENT', 'Extend the admitted action and adjustment evidence from retained five-year sources; unresolved records remain quarantined.', 'locked'],
      ['Model, strategy & Product', 'LOCKED', 'No V2 outcome screen, predictive model, strategy expression, Candidate authority, or option-performance claim exists.', 'locked'],
    ],
    ...
    factorQualification: 'Factor qualification V2', factorQualificationNote: 'The eight-factor catalog was calculated twice over the same frozen Dell population. The exact replay measures source and implementation fitness only; it never reads future returns.', factorQualificationBadge: 'REJECTED · EXACT REPLAY',
    factorStats: ['Registered factors', 'Complete vectors', 'Coverage', 'Pair checks', 'Near-duplicate groups'],
    factorCoverageMeaning: '266,335 of 437,402 declared paths produced complete eight-factor vectors. The 171,067 incomplete paths were explicitly quarantined; none were zero-filled. Availability was 60.89% across 161 eligible sessions, below the frozen 90% and chronology floors.',
    factorRedundancy: 'Strongest same-session relationship', factorRedundancyBody: 'The largest absolute weighted Spearman relationship was 0.8996 between the two risk guards. No pair met the full near-duplicate rule; redundancy is not the blocker and this says nothing about predictive value.',
    ...
    factorVerdict: 'Why V2 stopped', factorVerdictValue: '0 / 4 candidate Alpha admitted', factorVerdictBody: 'The registered source window begins on 17 Dec 2024, while the admitted split-action and adjustment evidence begins on 23 Jun 2025. Conservative benchmark quarantine closes the early windows; lowering gates or changing formulas would invalidate the test.',
    factorNext: 'Next bounded action', factorNextValue: 'REPAIR DATA · KEEP PROTOCOL FIXED', factorNextBody: 'Admit a versioned extension from the five-year split evidence already retained on Dell, preserve unresolved events as quarantine, and rerun this unchanged protocol. Only a passing qualification may authorize design of a finite Development outcome screen.',
    factorLimits: 'Qualification limits', factorLimitsBody: 'Membership is reconstructed rather than as operated; historical classifications and broad Regime diversity remain unproven; daily bars do not observe spreads or signed order flow; and split-neutral absence is not yet established.',
    ...
    resultCards: [['Factor Catalog V2', 'Blocked · split-evidence coverage'], ['Factor Screen V1', 'Closed · no candidate Alpha'], ['Pullback program', 'Closed · no stable selection'], ['Active models', 'None']],
  ...
  zh: {
    ...
    status: '因子发现 V2', statusValue: '数据资格诊断未通过',
    statusNote: '8项不读取结果的定义和一次精确重放已经完成。由于正式拆股证据晚于登记的127日数据窗口，冻结的90%覆盖门槛未通过；V2 尚未读取任何结果，也没有因子获准进入筛选。',
    ...
    readiness: '证据阶梯', readinessItems: [
      ...
      ['累计研究账本', '已更新', 'V1 的失败和唯一保留风险护栏继续计入；V2 明确披露其设计使用过这些开发期证据。', 'met'],
      ['因子目录 V2', '已登记', '8项精确定义覆盖延续、反转、收益时段、防御状态、流动性适用性与下行风险。', 'met'],
      ['V2 数据资格诊断', '未通过', '正式报告与重放完全一致，但按冻结的拆股证据规则，完整向量覆盖率只有60.89%。', 'locked'],
      ['历史拆股证据修复', '当前工作', '从戴尔已保留的五年来源扩展正式行动与复权证据；无法确认的记录继续隔离。', 'locked'],
      ['模型、策略与产品', '锁定', '目前没有 V2 结果筛选、预测模型、策略表达、个股候选权限或期权绩效主张。', 'locked'],
    ],
    ...
    factorQualification: '因子资格诊断 V2', factorQualificationNote: '8项因子已在戴尔同一冻结总体上完整计算两次。精确重放只检验来源与实现是否合格，全程不读取未来收益。', factorQualificationBadge: '未通过 · 精确重放',
    factorStats: ['登记因子', '完整向量', '覆盖率', '成对核验', '近重复组'],
    factorCoverageMeaning: '437,402条声明路径中，266,335条形成完整的8因子向量；171,067条不完整路径均明确隔离，没有静默补零。覆盖率为60.89%，共有161个合格交易日，低于冻结的90%覆盖与时间分布门槛。',
    factorRedundancy: '最强同日关系', factorRedundancyBody: '两个风险护栏之间的绝对加权 Spearman 关系最高，为0.8996；没有因子对满足完整的近重复规则。冗余不是本次阻塞原因，也不代表存在预测价值。',
    ...
    factorVerdict: 'V2 为什么停止', factorVerdictValue: '0 / 4 项候选 Alpha 获准', factorVerdictBody: '登记的数据窗口始于2024年12月17日，而获准使用的拆股行动与复权证据始于2025年6月23日。保守的基准隔离规则因此关闭早期窗口；降低门槛或修改公式都会破坏本次检验。',
    factorNext: '下一项有限动作', factorNextValue: '修复数据 · 协议保持不变', factorNextBody: '把戴尔已保留的五年拆股证据作为版本化扩展纳入，无法确认的事件继续隔离，然后以完全不变的协议重跑。只有资格诊断通过，才可以设计有限且预登记的开发期结果筛选。',
    factorLimits: '资格诊断边界', factorLimitsBody: '成员资格为事后重建而非当时实录；历史行业分类和充分的市场状态多样性尚未证明；日线不能观察点差或带方向订单流；拆股为空是否等于中性也尚未确立。',
    ...
    resultCards: [['因子目录 V2', '受阻 · 拆股证据覆盖'], ['因子筛选 V1', '已关闭 · 无候选 Alpha'], ['回撤研究项目', '已关闭 · 无稳定选择'], ['已激活模型', '无']],
  ...
  es: {
    ...
    status: 'DESCUBRIMIENTO DE FACTORES V2', statusValue: 'CALIFICACIÓN DE DATOS BLOQUEADA',
    statusNote: 'Las ocho definiciones sin resultados y una reproducción exacta están completas. El filtro congelado del 90 % falló porque la evidencia formal de splits empieza después de la ventana registrada de 127 sesiones. V2 no ha consultado resultados ni ha admitido factores.',
    ...
    readiness: 'Escalera de evidencia', readinessItems: [
      ...
      ['Registro acumulativo de investigación', 'ACTUALIZADO', 'Los fallos de V1 y su única salvaguarda conservada siguen contabilizados; V2 declara que su diseño se adapta a esa evidencia ya consumida.', 'met'],
      ...
      ['Calificación de datos V2', 'RECHAZADA', 'El informe y la reproducción coinciden exactamente, pero solo el 60,89 % de los vectores quedó completo bajo la regla congelada de splits.', 'locked'],
      ['Reparación histórica de splits', 'ACTUAL', 'Ampliar la evidencia formal con las fuentes de cinco años conservadas en Dell, manteniendo en cuarentena los casos no resueltos.', 'locked'],
      ['Modelo, estrategia y Producto', 'BLOQUEADOS', 'No existe selección de resultados V2, modelo predictivo, expresión de estrategia, autoridad sobre Candidatos ni afirmación sobre opciones.', 'locked'],
    ],
    ...
    factorQualification: 'Calificación de factores V2', factorQualificationNote: 'El catálogo de ocho factores se calculó dos veces sobre la misma población congelada en Dell. La reproducción exacta evalúa solo la aptitud de las fuentes y de la implementación; nunca consulta rendimientos futuros.', factorQualificationBadge: 'RECHAZADA · REPRODUCCIÓN EXACTA',
    factorStats: ['Factores registrados', 'Vectores completos', 'Cobertura', 'Pares revisados', 'Grupos casi duplicados'],
    factorCoverageMeaning: 'De 437.402 trayectorias declaradas, 266.335 produjeron vectores completos de ocho factores. Las 171.067 incompletas se pusieron en cuarentena explícita, sin rellenarlas con cero. La disponibilidad fue del 60,89 % en 161 sesiones aptas, por debajo de los filtros congelados de cobertura y cronología.',
    factorRedundancy: 'Relación contemporánea más intensa', factorRedundancyBody: 'La mayor relación de Spearman ponderada en valor absoluto fue 0,8996 entre las dos salvaguardas de riesgo. Ningún par cumplió la regla completa de casi duplicado; la redundancia no causó el bloqueo ni demuestra capacidad predictiva.',
    ...
    factorVerdict: 'Por qué se detuvo V2', factorVerdictValue: '0 / 4 candidatos de Alpha admitidos', factorVerdictBody: 'La ventana registrada comienza el 17 de diciembre de 2024, mientras que la evidencia formal de acciones y ajustes por splits empieza el 23 de junio de 2025. La cuarentena conservadora del índice cierra las ventanas iniciales; rebajar los filtros o cambiar fórmulas invalidaría la prueba.',
    factorNext: 'Siguiente acción acotada', factorNextValue: 'REPARAR DATOS · MANTENER EL PROTOCOLO', factorNextBody: 'Incorporar una ampliación versionada desde la evidencia de splits de cinco años ya conservada en Dell, mantener en cuarentena los eventos no resueltos y repetir el protocolo sin cambios. Solo una calificación aprobada permitirá diseñar una selección de Desarrollo finita y prerregistrada.',
    factorLimits: 'Límites de la calificación', factorLimitsBody: 'La composición está reconstruida y no registrada tal como se operó; la clasificación histórica y una diversidad amplia de regímenes no están demostradas; las barras diarias no observan diferenciales ni flujo firmado; tampoco se ha probado la ausencia neutral de splits.',
    ...
    resultCards: [['Catálogo de factores V2', 'Bloqueado · cobertura de splits'], ['Selección de factores V1', 'Cerrada · sin candidato de Alpha'], ['Programa Pullback', 'Cerrado · sin selección estable'], ['Modelos activos', 'Ninguno']],
  ...
  return <main className="research-page">
    ...
    <section className="research-card research-factor-qualification" aria-labelledby="research-factor-qualification-title">
      ...
      <div className="research-screen-verdict research-qualification-verdict">
        <article className="rejected"><span>{c.factorVerdict}</span><strong>{c.factorVerdictValue}</strong><p>{c.factorVerdictBody}</p></article>
    ...
    <section className="research-card research-factor-screening" aria-labelledby="research-factor-screening-title">
      <header><span>04</span><div><h2 id="research-factor-screening-title">{c.factorScreen}</h2><p>{c.factorScreenNote}</p></div><b>{c.factorScreenBadge}</b></header>
    ...
    <section className="research-card research-engineering-evidence" aria-labelledby="research-engineering-title">
      <header><span>05</span><div><h2 id="research-engineering-title">{c.engineering}</h2><p>{c.engineeringNote}</p></div><b>{c.engineeringBadge}</b></header>
    ...
    <section className="research-card research-model-registry" aria-labelledby="research-model-title">
      <header><span>06</span><div><h2 id="research-model-title">{c.registry}</h2><p>{c.featured}</p></div></header>
    ...
    <section className="research-card research-lifecycle"><header><span>07</span><h2>{c.lifecycle}</h2></header><ol>{c.stages.map((stage, index) => <li className={index === 0 ? 'active' : 'locked'} key={stage}><i>{index + 1}</i><strong>{stage}</strong><small>{index === 0 ? c.current : c.locked}</small></li>)}</ol></section>

    <section className="research-card research-results"><header><span>08</span><div><h2>{c.results}</h2><p>{c.resultsNote}</p></div></header><div className="research-result-grid">{c.resultCards.map(([name, state]) => <article key={name}><b aria-hidden="true">—</b><strong>{name}</strong><span>{state}</span></article>)}</div></section>
