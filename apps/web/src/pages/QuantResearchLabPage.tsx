...
const COPY = {
  en: {
    ...
    readiness: 'Evidence ladder', readinessItems: [
      ...
      ['Next discovery campaign', 'INPUT QUALIFICATION', 'Five ideas are retained: three Alpha interactions and one risk interaction advance; one near-duplicate was rejected before outcomes.', 'active'],
    ...
    cycleStats: ['Operating mode', 'Current stage', 'Completed campaigns', 'Formal trials consumed', 'Next campaign'], cycleStatValues: ['RENEWABLE', 'INPUT QUALIFICATION', '2', '14', 'NOT REGISTERED'],
    cycleStages: [
      ...
      ['02', 'Build without outcomes', 'CURRENT', 'Align the four accepted designs and qualify timing, coverage, variation, concentration, and redundancy without labels.'],
    ...
    cycleCurrent: 'Current position', cycleCurrentValue: '4 DESIGNS · INPUT REVIEW', cycleCurrentBody: 'Three candidate-Alpha interactions and one risk interaction are frozen for outcome-blind qualification. They are not registered return trials and have no performance result.',
    cycleInfrastructure: 'Pre-campaign foundation', cycleInfrastructureValue: '1 QUALIFIED MARKET-STATE PANEL', cycleInfrastructureBody: 'The Market-State panel is registered for exact outcome-blind reuse. Population, features, controls, and labels are not claimed as materialized here; controls and labels remain inaccessible.',
    ...
    agentPilot: 'Stage-isolated research team', agentPilotNote: 'The supervised multi-Agent pilot has completed market-state qualification plus hypothesis intake and deduplication. Four designs now enter outcome-blind input review; outcomes remain closed.', agentPilotBadge: 'HYPOTHESES FROZEN · INPUT REVIEW',
    agentStats: ['Market-state qualification', 'Exact replay', 'Accepted designs', 'Validation / Holdout'], agentStatValues: ['PASSED · 267 / 287', 'MATCHED', '4 · NO OUTCOMES', 'SEALED'],
    agentStages: [
      ...
      ['03', 'Hypotheses and input review', 'CURRENT', 'Five cards are frozen; four accepted designs must pass outcome-blind alignment and exact replay before protocol and ledger registration.'],
    ...
    screeningV2Next: 'Next governed question', screeningV2NextValue: '4 DESIGNS · INPUT REVIEW', screeningV2NextBody: 'Three Alpha interactions and one risk interaction are frozen for outcome-blind qualification. Coverage, alignment, episode support, and exact replay must pass before protocol registration or Development outcomes.',
  ...
  zh: {
    ...
    readiness: '证据阶梯', readinessItems: [
      ...
      ['下一批因子发现', '输入资格审查', '共保留5个想法：3个 Alpha 交互和1个风险交互进入下一步，另1个近重复想法在读取结果前被拒绝。', 'active'],
    ...
    cycleStats: ['运行模式', '当前阶段', '已完成批次', '已消耗正式试验', '下一批次'], cycleStatValues: ['持续循环', '输入资格审查', '2', '14', '尚未登记'],
    cycleStages: [
      ['01', '提出假设并去重', '已完成', '共保留5个想法；4个进入下一步，1个近重复想法在任何结果访问前停止。'],
      ['02', '不读取结果地构建', '当前', '对齐4个已接受设计，并在不接触标签的情况下审查时点、覆盖、变异、集中度和冗余。'],
      ['03', '预登记并反证', '锁定', '在打开开发期标签前，冻结有限试验预算、结果定义、成本、重复检验、门槛、入选上限与停止规则。'],
      ['04', '重放、关闭并返回', '锁定', '攻击泄漏与稳定性，精确复现结果，追加全部失败，分流幸存证据，然后返回新假设入口。'],
    ],
    cycleCurrent: '当前位置', cycleCurrentValue: '4项设计 · 输入审查', cycleCurrentBody: '3个候选 Alpha 交互和1个风险交互已冻结，正等待结果盲资格审查；它们还不是正式收益试验，也没有绩效结果。',
    cycleInfrastructure: '批次前基础', cycleInfrastructureValue: '1个合格市场状态面板', cycleInfrastructureBody: '市场状态面板已登记为可精确复用的结果盲输入。本页不宣称研究总体、特征、控制项或标签已经实体化；控制项和标签仍不可访问。',
    ...
    agentPilot: '阶段隔离的研究协作组', agentPilotNote: '人工监督的多 Agent 试点已完成市场状态资格审查、假设登记与去重。4项设计现进入结果盲输入审查，结果读取仍保持关闭。', agentPilotBadge: '假设已冻结 · 输入审查',
    agentStats: ['市场状态资格', '精确重放', '接受的设计', 'Validation / Holdout'], agentStatValues: ['通过 · 267 / 287', '完全一致', '4项 · 未读结果', '保持密封'],
    agentStages: [
      ['01', '独立审查', '已完成', '治理、方法和实现审查在真实运行前发现并修正了实质问题。'],
      ['02', '市场状态资格审查', '已完成', '287个交易日全部完成；重建宽度联合可用267日，独立规范字节重放完全一致。'],
      ['03', '假设与输入审查', '当前', '5张假设卡已经冻结；4项设计必须先通过结果盲对齐与精确重放，之后才能登记协议和账本。'],
    ...
    screeningV2Next: '下一项受控问题', screeningV2NextValue: '4项设计 · 输入审查', screeningV2NextBody: '3个 Alpha 交互和1个风险交互已冻结，等待结果盲资格审查。覆盖、对齐、状态片段支持和精确重放必须先通过，之后才能登记协议或打开开发期结果。',
  ...
  es: {
    ...
    readiness: 'Escalera de evidencia', readinessItems: [
      ...
      ['Próxima campaña', 'CALIFICACIÓN DE ENTRADAS', 'Se conservan cinco ideas: tres interacciones Alpha y una de riesgo avanzan; una casi duplicada fue rechazada antes de consultar resultados.', 'active'],
    ...
    cycleStats: ['Modo operativo', 'Fase actual', 'Campañas concluidas', 'Pruebas formales consumidas', 'Próxima campaña'], cycleStatValues: ['RENOVABLE', 'CALIFICACIÓN DE ENTRADAS', '2', '14', 'NO REGISTRADA'],
    cycleStages: [
      ...
      ['02', 'Construir sin resultados', 'ACTUAL', 'Alinear los cuatro diseños aceptados y calificar calendario, cobertura, variación, concentración y redundancia sin etiquetas.'],
    ...
    cycleCurrent: 'Posición actual', cycleCurrentValue: '4 DISEÑOS · REVISIÓN DE ENTRADAS', cycleCurrentBody: 'Tres interacciones Alpha candidatas y una de riesgo están congeladas para calificación sin resultados. Aún no son pruebas formales ni tienen métricas de rendimiento.',
    cycleInfrastructure: 'Base previa a la campaña', cycleInfrastructureValue: '1 PANEL DE ESTADO CALIFICADO', cycleInfrastructureBody: 'El panel de estado de mercado está registrado para reutilización exacta sin resultados. Aquí no se afirma que población, variables, controles o etiquetas estén materializados; controles y etiquetas siguen inaccesibles.',
    ...
    agentPilot: 'Equipo de investigación aislado por etapas', agentPilotNote: 'El piloto multiagente supervisado completó la calificación del estado de mercado, el registro de hipótesis y la deduplicación. Cuatro diseños pasan ahora a revisión de entradas sin resultados.', agentPilotBadge: 'HIPÓTESIS CONGELADAS · REVISIÓN',
    agentStats: ['Calificación de mercado', 'Reproducción exacta', 'Diseños aceptados', 'Validación / Holdout'], agentStatValues: ['SUPERADA · 267 / 287', 'COINCIDE', '4 · SIN RESULTADOS', 'SELLADOS'],
    agentStages: [
      ...
      ['03', 'Hipótesis y revisión de entradas', 'ACTUAL', 'Cinco fichas están congeladas; los cuatro diseños aceptados deben superar alineación y reproducción sin resultados antes de registrar protocolo y libro de pruebas.'],
    ...
    screeningV2Next: 'Siguiente pregunta gobernada', screeningV2NextValue: '4 DISEÑOS · REVISIÓN DE ENTRADAS', screeningV2NextBody: 'Tres interacciones Alpha y una de riesgo están congeladas para calificación sin resultados. Cobertura, alineación, soporte temporal y reproducción exacta deben aprobarse antes del protocolo o de abrir Desarrollo.',
  ...
  return <main className="research-page">
    ...
    <section className="research-card research-engineering-evidence" aria-labelledby="research-cycle-title">
      ...
      <ol className="research-engineering-rail">
        {c.cycleStages.map(([index, title, state, body], stageIndex) => <li className={stageIndex === 0 ? 'done' : stageIndex === 1 ? 'current' : 'blocked'} key={title}><i>{stageIndex === 0 ? '✓' : stageIndex === 1 ? '●' : '→'}</i><strong>{index} · {title}</strong><small>{state} · {body}</small></li>)}
      </ol>
    ...
    <section className="research-card research-engineering-evidence research-agent-pilot" aria-labelledby="research-agent-pilot-title">
      ...
      <ol className="research-engineering-rail research-agent-rail">
        {c.agentStages.map(([index, title, state, body], stageIndex) => <li className={stageIndex < 2 ? 'done' : stageIndex === 2 ? 'current' : 'blocked'} key={title}><i>{stageIndex < 2 ? '✓' : stageIndex === 2 ? '●' : '→'}</i><strong>{index} · {title}</strong><small>{state} · {body}</small></li>)}
      </ol>
