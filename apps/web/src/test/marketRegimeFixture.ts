...
function relationship(index: number): Relationship {
  ...
  return {
    ...
    current: {
      relationship_state: state, confidence: 'low', availability: 'available', missing_reason: null,
...
function universe(id: string, display: string, count: number, score: string) {
  ...
  return { definition, composite: { regime_score: score, dimensions, reason_codes: ['composite_available'], logical_fingerprint: 'c'.repeat(64) }, current_state: { composite: score, instantaneous_candidate_state: 'balanced', confirmed_state: 'balanced', transition_status: 'held', pending_target_state: null, confirmation_sessions_remaining: 0, in_hysteresis_band: false, supporting_dimension_ids: ['trend', 'volatility', 'leadership_dispersion'], conflicting_dimension_ids: ['liquidity_participation'], threshold_distances: [{ threshold_id: 'balanced_to_risk_on', threshold: '70.0000', signed_distance: '-6.0898', boundary_operator: '>=' }], reason_codes: ['state_input_available', 'candidate_band_balanced', 'confirmed_state_held'] }, current_state_explanation: { candidate_band_text: 'Composite maps to Balanced.', transition_text: 'Confirmed state held.', disclaimers: ['Not a recommendation.'], reason_codes: ['confirmed_state_held'] }, state_history: [], dimension_explanations: [] };
}
