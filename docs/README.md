...
- [2026-09-09 Daily EOD Publication and Deployment Audit](audits/daily-eod-publication-deployment-2026-09-09.md): exact 2026-09-08 acquisition, persistent-workspace analytics, publication, deployment-path recovery, final OCI release, and postflight evidence.
- [2026-09-09 Daily Universe Membership Publication Audit](audits/daily-universe-membership-publication-2026-09-09.md): corrected catalog binding, prospective 2026-09-08 candidate, exact Plan/Apply, zero-write postflight, and unchanged research boundary.
- [2026-09-09 Daily Universe Membership Sidecar Audit](audits/daily-universe-membership-sidecar-2026-09-09.md): read-only sidecar state machine, Pipeline Wake 2.1 projection, real Dell completion proof, and unchanged serving/timer authority.
- [2026-09-08 Cross-Strategy Decision Desk Deployment Audit](audits/cross-strategy-decision-desk-deployment-2026-09-08.md): stable-ID joined strategy review, family-specific research gates, exact Snapshot/bundle identities, OCI postflight, and final Dell reconciliation.
...
- [Candidate Segmented Append V1](data-contracts/opportunity-candidate-segmented-append-v1.md): exact one-session successor proof over the versioned segmented chain.
- [Candidate Segmented Session Candidate V1](data-contracts/opportunity-candidate-segmented-session-candidate-v1.md): emits one non-authoritative daily segment directly from validated current objects without a cumulative V1 reread.
- [ADR 0157: Direct Candidate session emission](decisions/0157-emit-candidate-session-segment-from-current-objects.md): binds the direct payload to its exact parent, intended V1 audit, and incremental validation while preserving separate semantic audit.
- [ADR 0158: Direct Candidate append composition](decisions/0158-compose-direct-candidate-session-into-successor-append.md): verifies intended V1 physical completion and composes the exact direct session bytes into a versioned successor without cumulative semantic replay.
- [ADR 0159: Multi-generation Candidate lineage](decisions/0159-validate-ordered-multigeneration-candidate-append-lineage.md): validates an explicit ordered append sequence and advances the exact parent manifest, source audit, session count, and chain tip.
- [ADR 0160: Candidate chain-head checkpoint](decisions/0160-checkpoint-segmented-candidate-chain-head.md): makes an expected immutable head plus one append exactly reproduce full-lineage cold identity without reading historical segments.
- [Candidate Segmented Chain Head V1](data-contracts/opportunity-candidate-segmented-chain-head-v1.md): small expected-identity checkpoint, incremental lineage fingerprint, custody, and explicit non-authority boundary.
- [ADR 0161: Candidate chain-head publication plan](decisions/0161-plan-segmented-candidate-chain-head-publication.md): plans a content-addressed head and pointer-last switch under bounded-family CAS, with rollback/recovery/retention policy and zero canonical writes.
- [Candidate Segmented Chain-Head Publication V1](data-contracts/opportunity-candidate-segmented-chain-head-publication-v1.md): immutable target, current pointer, successor, CAS, rollback, recovery, retention, and no-authority contracts.
- [ADR 0162: Disconnected Candidate chain-head Apply/recovery](decisions/0162-prove-disconnected-candidate-chain-head-apply-recovery.md): proves exact release-first/pointer-last execution, interruption recovery, and zero-write postflight only in `/tmp`, while refusing the Production root.
- [Candidate Segmented Chain-Head Apply V1](data-contracts/opportunity-candidate-segmented-chain-head-apply-v1.md): exact execution binding, recovery classification, durability, idempotency, result semantics, and explicit non-Production authority.
- [ADR 0163: Candidate chain-head full-lineage audit](decisions/0163-audit-candidate-chain-head-against-full-lineage.md): periodically cold-reads the complete lineage and proves it exactly reproduces the expected active head under before/after current-state CAS.
- [Candidate Segmented Chain-Head Full-Lineage Audit V1](data-contracts/opportunity-candidate-segmented-chain-head-full-lineage-audit-v1.md): deterministic inputs, exact-match result, daily/periodic/code-change policy, failure handling, and explicit zero authority.
...
- [Canonical Universe Membership Publication](operations/universe-membership-publication.md): exact-plan physical-first/marker-last Apply, governed read boundary, recovery, and postflight procedure.
- [ADR 0153: Physical-first, marker-last Membership publication](decisions/0153-publish-membership-physical-first-marker-last.md): makes the logical marker the only completion boundary and adds fail-closed recovery.
