...
Activation V2 preserves an explicit public order—Primary/Common Shares first and Secondary/Common Shares + ADRs second—rather than sorting stable IDs lexically. Immutable-target publication, verify-then-link recovery, and rollback share the same active-pointer lock and compare-and-swap boundary. Newly created publication and pointer directories are durably fsynced with their parent entries before a target or pointer can be treated as completed.
