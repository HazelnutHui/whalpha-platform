...
Phase B2A implemented and fixture-validated the boundary offline. Phase B2B implements a bounded streaming transport and atomic source-cache, observation, canonical-evidence, and logical-completion layers. Six authorized B2B runs failed closed during official Series/Class CSV discovery and produced no completed source cache or evidence snapshot. The sixth run confirmed a design error: discovery required every historical CSV path to match an exact download template before it selected the unique latest release. The offline selector now separates baseline URL safety from exact dataset-template authorization, chooses the unique latest cutoff candidate first, and requires the full exact rule only for that selected source. The observed 2022 path remains outside the allowlist and can only be a non-download historical warning under the strict conditions below. The source-cache publication contract and submissions ZIP safety gate are now also complete under local synthetic tests. No post-remediation live run has occurred. Universe activation and Dashboard changes remain out of scope.
