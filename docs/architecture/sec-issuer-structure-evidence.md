...
Official landing pages may list multiple yearly releases. CSV discovery is therefore row-semantic rather than link-order based: each candidate must carry an explicit dataset year, update/effective date, CSV format label, and allowlisted `https://www.sec.gov/files/` URL. Candidates later than the evidence cutoff are excluded; the newest remaining date is selected. Distinct URLs tied at that date, missing or malformed dates, unsafe URLs, and pages without an eligible candidate fail closed. Candidate totals and the selected year/date are persisted without contact configuration.
