...
Price and USD 20M comparisons therefore cannot set or silently ignore `Inexact`/`Rounded`. Material-return ratio gates use cross-multiplied coefficients. Audit-only return analytics can require non-terminating division; those calculations run under an explicit local precision of 78 (`2 × decimal128 precision 38 + 2`) with `ROUND_HALF_EVEN`, so process precision, rounding mode, import order, and test order do not change their result. A separate `fractions.Fraction` dry-run oracle reconciles every available daily product, complete-window median, decision, membership, and immutable V1 median before publication can be considered.
