...
Implemented through Snapshot 1.11 / Dashboard 2.8, including the split
Candidate consumer, lazy Strategy Channel and Visual Context products, and the
lazy Sector Rotation resource. Exact active Production identity belongs in
[current context](../project/current-context.md). Earlier releases remain
immutable readable historical/rollback contracts, not current-state authority.
...
and multilingual browser support are implemented. ADR 0058 additionally implements
