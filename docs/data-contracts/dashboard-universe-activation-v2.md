...
The V2 catalog has a contract-defined order independent of lexical IDs: `Common Shares` is first and remains the sole default; `Common Shares + ADRs` is second. Legacy remains hidden. The source-derived memberships are 1,718 CS and 1,831 total (1,718 CS + 113 ADRC). The bounded activation has been applied; exact active and rollback fingerprints belong in [current context](../project/current-context.md).
