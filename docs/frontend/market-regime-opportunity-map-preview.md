...
The deployed frontend reads an immutable Snapshot rather than `/tmp`. The
Daily Decision Brief, decision-lane highlights, collapsed audit table,
second-workspace rename, and guest entry are deployed. Guest and credential
Sessions consume the identical analytics payload with the same fields,
precision, freshness, and as-of session. Exact active release and contract
state belong in [current context](../project/current-context.md).
