...
All downloaded sources must pass content, size, hash, and ZIP safety validation before atomic source-cache publication. The private source cache has exactly nine provenance artifacts: two official ticker JSON files, three official landing HTML pages, three selected CSV files, and `submissions.zip`. Landing HTML must have an approved HTML Content-Type, remain within the response bound, and produce the expected unique `File / Format / Size` table signature. It is retained for private audit only and must never be served as Dashboard data.

Every artifact manifest record includes its role, official URL, byte size, and SHA-256. The manifest excludes request/response headers, User-Agent, contact identity, cookies, Authorization, and credentials. Before publication, staging must contain exactly the nine expected regular non-symlink artifacts; every size and hash is reread, and the completion manifest is written last. The completed staging inventory is then atomically renamed. Existing or symlink targets are rejected, and any failure removes only the current staging directory.

The submissions archive gate rejects encrypted, duplicate or normalized-duplicate, absolute, traversing, backslash, percent-encoded, symlink, non-regular, nested, or non-`CIK##########.json` members. It enforces member-count, per-member expansion, total expansion, and compression-ratio bounds, handles zero compressed-size metadata without division by zero, reads each member with a bounded stream, and requires parseable JSON with filename-consistent CIK and a basic SEC filings structure. It writes no extracted member to disk. Normalized inputs must reconcile completely; ambiguous canonical mappings, stable-identity collisions, canonical business-key conflicts, future-filing leakage, schema mismatch, fingerprint mismatch, or staging residue prevent evidence publication. The logical completion manifest is written last.
...
## Offline Source-Cache and ZIP Contract Completion

The source cache now treats all three official landing pages as private provenance evidence and reconciles exactly nine artifacts rather than discarding the HTML or treating it as Dashboard data. Artifact roles, official URLs, sizes, and hashes are verified before the last-written completion manifest and atomic rename. Synthetic tests cover successful publication, manifest secrecy, landing Content-Type/signature validation, existing/symlink target rejection, and failure cleanup.

The submissions ZIP gate now covers encryption, exact and normalized duplicates, absolute/traversing/backslash/percent-encoded names, links and other non-regular entries, explicit flat CIK naming, member-count and expansion limits, compression ratio and zero-size handling, bounded reads, JSON parsing, CIK consistency, and basic filings schema. This work was completely offline: no credential was read or statted, `/data` and OCI were not accessed, and no SEC/Massive/other external request, snapshot, bundle, deployment, or Universe activation occurred. A live SEC run remains unauthorized and was not executed.

## Final Bounded B2B Run

The last authorized run for this product stage invoked `--apply` exactly once from `2026-08-16T10:39:44Z` through `2026-08-16T10:39:51Z` and exited 1. It made six SEC requests and zero retries. Strict serial control flow identifies the attempts as the two ticker JSON resources, Series/Class landing, selected Series/Class CSV, CEF landing, and selected CEF CSV. Reaching request 5 proves Series/Class landing selection and its selected CSV validation completed; reaching request 6 proves CEF landing produced a validated selection. No BDC landing/CSV or submissions request occurred.

The failure was retained only as `sec_transport_or_source_validation_failure`. The 329-byte operation diagnostic has schema `1.0`, request/retry counts, and an empty `quality_summary`; source-cache cleanup removed the successful landing selections and downloaded staging files. It therefore does not prove the Series/Class or CEF candidate counts, warning counts, selected date/path, sixth-response Content-Type/header, or the narrower transport/source-validation subcondition. No inference or rule change is permitted from that missing evidence, and the entrypoint must not be rerun.

All four completed targets remained absent, staging residue was zero, and the protected 34-file inventory remained 12,942,699 bytes with digest `398d3c8eb8a986ffc34a7f2fe19c50961eca0d9ed52bd980da95217561333f17`. No observation, canonical evidence, logical manifest, Core/Broad shadow audit, snapshot, bundle, deployment, or Universe activation was produced. SEC B2 is paused for this product stage; work returns to EOD history, trailing liquidity, sector taxonomy, and website functionality.
