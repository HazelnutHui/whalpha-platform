...
| SEC projection readiness | ADR 0224 keeps facts at filer grain. The conservative single-common-stock CIK class covers 6,122,451 / 6,244,758 admitted common-stock row-sessions (~98.0%); 122,307 rows in multi-common-stock groups remain outside the default. Only 11 / 1,255 link sessions pass the retained next-open observation-time gate; reconstructed history is development-only, not validation or performance authority |
