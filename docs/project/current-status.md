...
For analysis session 2026-08-14, exchange-calendars 4.13.2 computed 20 sessions from 2026-07-17 through 2026-08-13. Fifteen sessions are completed through 08-04 plus 08-12 and 08-13; five are missing and none corrupt. Candidate A observation counts are 1,742 at 15 sessions, one at 14, one at 13, one at eight, two at six, two at five, one at two, and one at one; Candidate B counts are 1,854, two, one, one, two, two, one, and one. All 1,751/1,864 results remain `insufficient_history`; 20/20, 19/20, missing-previous, zero-volume, and emitted median counts are zero. No current-day bar entered the window.

The remaining planning-only gap is five same-day identity/resolver snapshots plus five Grouped Daily sessions: approximately 75 requests and conservative ceiling 105, with zero retries, fixed 15-second spacing, and two possible batches of at most three. No later date or batch is authorized.
...
Keep SEC B2 paused, Legacy production membership unchanged, and the 20-session readiness at `insufficient_history`. Manually review the completed fourth batch before separately deciding whether to authorize the next chronological batch of at most three sessions; no later batch is implied by this result.
