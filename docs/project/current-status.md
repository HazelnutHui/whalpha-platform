...
For analysis session 2026-08-14, exchange-calendars 4.13.2 computed 20 sessions from 2026-07-17 through 2026-08-13. Twelve sessions are completed through 07-30 plus 08-12 and 08-13; eight are missing and none corrupt. Candidate A observation counts are 1,742 at 12 sessions, one at 11, one at 10, one at five, two at three, three at two, and one at one; Candidate B counts are 1,855, one, one, one, two, three, and one. All 1,751/1,864 results remain `insufficient_history`; 20/20, 19/20, missing-previous, zero-volume, and emitted median counts are zero. No current-day bar entered the window.

The remaining planning-only gap is eight same-day identity/resolver snapshots plus eight Grouped Daily sessions: approximately 120 requests and conservative ceiling 168, with zero retries, fixed 15-second spacing, and three possible batches of at most three. No later date or batch is authorized.
...
Keep SEC B2 paused, Legacy production membership unchanged, and the 20-session readiness at `insufficient_history`. Manually review the completed third batch before separately deciding whether to authorize the next chronological batch of at most three sessions; no later batch is implied by this result.
