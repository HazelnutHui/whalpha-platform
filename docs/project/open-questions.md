...
- Which provider rows caused the four ambiguous/nonzero mapped-business-key conflicts in the failed Phase B1 run? Raw payload was intentionally not retained, so a future authorized run must emit safe conflict diagnostics without raw persistence.
