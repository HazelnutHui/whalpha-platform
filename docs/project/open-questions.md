...
- The four failed-run ambiguities are resolved: `BCPC` and `TPC` each had one stable-ID resolved observation and one identifier-free excluded observation incorrectly attached by ticker fallback. The remaining open operational question is whether the next separately authorized run passes the corrected gates and produces sufficient provider security-form coverage.
