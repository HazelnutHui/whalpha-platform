...
- Research data: keep formulas frozen until permission-cleared point-in-time
  ...
  aggregate-only operation. It is simulation-tested.
  The exact run then filled all six pages/6,000 rows and remained truncated.
...
- Research falsification: ADR 0105 now independently reconciles descriptive
  ...
  the external durable one-use holdout reservation seam with fixtures. ADR 0108
...
- Product: preserve leadership quality and entry location as separate axes.
  ...
  from its one in-memory panel. Market Intelligence 1.3 and Approval Plan 1.3
...
- [x] Add default-off one-shot Dashboard Snapshot Apply reservation, exact
  active-state proof, and no-write interruption recovery
...
ordinary-fresh, lag-zero `2026-09-01T123500Z-579a26759a9e` source-bound
Snapshot 1.11 / Dashboard 2.8
